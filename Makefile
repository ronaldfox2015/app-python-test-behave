...
tast: ## installar: make tast EXECUTE=install
	docker run -it -v "$(PWD)/app:/app" -w "/app" $(IMAGE_DEPLOY) pip install git+https://github.com/behave/behave
...
start: ## inicializar proyecto: make login
	docker run -it -p 80:80 --name $(PROJECT_NAME)  -v "$(PWD)/test:/behave" -w "/behave"  $(IMAGE_DEPLOY) behave
...
ssh: ## inicializar proyecto: make ssh
	docker run -it -p 80:80 --name $(PROJECT_NAME) -v "$(PWD)/app:/app" -w "/app"  $(IMAGE_DEPLOY) bash
